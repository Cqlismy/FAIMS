...
{
    m_ui->setupUi(this);
    setWindowTitle(QStringLiteral("Configure"));
    setWindowIcon(QIcon(":/images/settings.png"));

    m_ui->baudrateBox->setInsertPolicy(QComboBox::NoInsert);
    connect(m_ui->applyButton, &QPushButton::clicked, this, &SettingDialog::apply);
    connect(m_ui->searchButton, &QPushButton::clicked, this, &SettingDialog::search);
...
void SettingDialog::showPortInfo(int idx)
{
    ...
    m_ui->vidlabel->setText(tr("Vendor ID:%1").
                                    arg(list.count() > 5 ? list.at(5) : tr(blankString)));
    m_ui->pidlabel->setText(tr("Product ID:%1").
...
void SettingDialog::fillPortsInfo(void)
{
...
void SettingDialog::fillPortsParameters(void)
{
...
void SettingDialog::updateSettings(void)
{
...
void SettingDialog::search(void)
{
    fillPortsInfo();
}
